...
isHoliday date =
  ...
  in (month, dayOfMonth) `elem` holidayDates ||
     case goodFriday of
       Just gd -> (month, dayOfMonth) == toGregorian gd
       Nothing -> False
     ||
     case thanksgiving of
       Just tg -> (month, dayOfMonth) == toGregorian tg
       Nothing -> False
     ||
     case blackFriday of
       Just bf -> (month, dayOfMonth) == toGregorian bf
       Nothing -> False
...
main = do
  ...
  inputDate <- getLine
  let [inputYear, inputMonth, inputDay] = map read (words inputDate)
      date = fromGregorian (fromIntegral inputYear) (fromIntegral inputMonth) (fromIntegral inputDay)
  if isHoliday date
