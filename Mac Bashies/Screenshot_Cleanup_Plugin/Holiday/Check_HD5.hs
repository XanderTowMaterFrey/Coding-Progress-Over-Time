...
isHoliday date =
  ...
  in do
    ...
    putStrLn $ "Year, Month, Day: " ++ show (year, month, dayOfMonth)
    putStrLn $ "Weekday: " ++ show weekday
    putStrLn $ "Holiday Dates: " ++ show holidayDates
    putStrLn $ "Good Friday: " ++ show goodFriday
    putStrLn $ "Thanksgiving: " ++ show thanksgiving
    putStrLn $ "Black Friday: " ++ show blackFriday
    (month, dayOfMonth) `elem` holidayDates
    || case goodFriday of
      Just gd -> do
        putStrLn $ "Good Friday (Just): " ++ show gd
        putStrLn $ "Comparison Result: " ++ show ((month, dayOfMonth) == toGregorian gd)
        (month, dayOfMonth) == toGregorian gd
      Nothing -> False
    || case thanksgiving of
      Just tg -> do
        putStrLn $ "Thanksgiving (Just): " ++ show tg
        putStrLn $ "Comparison Result: " ++ show ((month, dayOfMonth) == toGregorian tg)
        (month, dayOfMonth) == toGregorian tg
      Nothing -> False
    || case blackFriday of
      Just bf -> do
        putStrLn $ "Black Friday (Just): " ++ show bf
        putStrLn $ "Comparison Result: " ++ show ((month, dayOfMonth) == toGregorian bf)
        (month, dayOfMonth) == toGregorian bf
      Nothing -> False
...
main = do
  ...
  let date = fromGregorian (fromIntegral year) (fromIntegral month) (fromIntegral day)
  if isHoliday date
