...
1 ask if its not alphabet \ 

    T.head inputText != isAlpha 

    true -> appends -> rotateIntake tail

    | (not $ isAlpha $ T.head inputText) = T.append (T.head inputText) (rotateIntake rotateBy (T.tail inputText)) 
    -- this says hey i am not a abc char, that means you gotta do something, so, what to do 



    | T.filter 
    (T.head inputText) == not isAlpha T.head inputText
    T.head inputText != (isAlpha $ T.head inputText) inputText

        = T.append (T.head inputText) (rotateIntake rotateBy (T.tail inputText))
    ...
    | (otherwise) = we should handle non alpha 
...
rotateIntake rotateBy inputText
    ...
    -- $ T.toUpper T.singleton (T.tail inputText) (rotate rotateBy (T.tail inputText)))
    -- | (T.head inputText == T.head (T.toLower inputText)) = T.append $ T.singleton (T.toLower $ rotate rotateBy (T.tail inputText)) (rotate rotateBy (T.tail inputText))
    | otherwise = T.empty --T.append (T.singleton $ T.head inputText) (rotateIntake rotateBy (T.tail inputText))
